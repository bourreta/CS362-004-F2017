...
int main(){
    ...
    int result;
    int failureCount = 0;
    int preVillageDiscard, postVillageDiscard, preVillagePlayed, postVillagePlayed;
    ...
    */
    ...
    for(j = 1; j <= n; j++){
        ...
        preVillageDiscard = 0;
        postVillageDiscard = 0;
        preVillagePlayed = 0;
        postVillagePlayed = 0;
        
        randomizeGameState(&G);
        G.hand[G.whoseTurn][G.handCount[G.whoseTurn]] = village; //add village to player 0 hand
        G.handCount[G.whoseTurn]++;
        
        for(i = 0; i < G.discardCount[G.whoseTurn]; i++){         //count initial discard
            if(G.discard[G.whoseTurn][i] == village) {preVillageDiscard++;}
        }
        
        for(i = 0; i < G.playedCardCount; i++){         //count initial
            if(G.playedCards[i] == village) {preVillagePlayed++;}
        }
        memset(&pre, 0, sizeof(struct gameState));   // clear the game state
        memcpy(&pre, &G, sizeof(struct gameState)); //copy gameState into pre
        
        result = cardEffect(village, 0, 0, 0, &G, G.handCount[G.whoseTurn] - 1, NULL);
        
        for(i = 0; i < G.discardCount[G.whoseTurn]; i++){         //count initial discard
            if(G.discard[G.whoseTurn][i] == village) {postVillageDiscard++;}
        }
        
        for(i = 0; i < G.playedCardCount; i++){         //count initial
            if(G.playedCards[i] == village) {postVillagePlayed++;}
        }
...
#endif
        if(((G.discardCount[G.whoseTurn] > pre.discardCount[pre.whoseTurn]) || (G.playedCardCount > pre.playedCardCount)) && ((postVillagePlayed > preVillagePlayed) || (postVillageDiscard > preVillageDiscard))){
#if (NOISY_TEST == 1)
            printf("\t\tPASS: Village card was placed in played/discard pile\n");
#endif
        }
        else{
#if (NOISY_TEST == 1)
            printf("\t\tFAIL: Village cards in discard(post-call: %d, village cards in played card pile(post-call: %d \n", postVillageDiscard, postVillagePlayed);
            printf("\t\tVillage cards in discard(pre-call): %d, village cards in played card pile(pre-call): %d \n", preVillageDiscard, preVillagePlayed);
            ...
            printf("\t\tPlayed card count(post-call): %d, played card count(pre-call): %d\n", G.playedCardCount, pre.playedCardCount);
#endif
            failureCount++;
        }
    }
